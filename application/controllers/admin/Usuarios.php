...
class Usuarios extends Admin_Controller {
    ...
    private function salvar_usuario($id = null) {
        ...
        $this->form_validation->set_rules('nivel', 'Nível', 'required|in_list[admin,atendente]');
        ...
        if ($id) {
            ...
            if ($this->Usuario_model->update($id, $dados)) {
                // Salvar permissões (apenas para atendentes)
                if ($dados['nivel'] == 'atendente') {
                    $this->salvar_permissoes($id);
                } else {
            ...
            if ($novo_id) {
                // Salvar permissões (apenas para atendentes)
                if ($dados['nivel'] == 'atendente') {
                    $this->salvar_permissoes($novo_id);
                }
     ...
     */
    private function salvar_permissoes($usuario_id) {
        ...
        foreach ($permissoes_post as $modulo => $acoes) {
            ...
            $this->Usuario_model->insert_permissao($usuario_id, $modulo, $permissoes_json);
        }
    }
